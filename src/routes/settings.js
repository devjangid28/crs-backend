...
router.put('/', async (req, res, next) => {
  try {
    const body = req.body.company_info || req.body;
    const {
      companyName, company_name, address, city, state, country, pincode,
      phone, email, website, gstVat, gst_vat, taxId, tax_id,
      currency, logo, timezone
    } = body;
    ...
    if (parseInt(rows.rows[0].cnt) === 0) {
      await query(
        `INSERT INTO store_settings (company_name, address, city, state, country, pincode, phone, email, website, gst_vat, tax_id, currency, logo, timezone, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [companyName || company_name || 'CRS Repair Shop', address || null,
         city || null, state || null, country || null, pincode || null,
         phone || null, email || null, website || null,
         gstVat || gst_vat || null, taxId || tax_id || null,
      ...
      for (const [front, db] of Object.entries(mapping)) {
        if (body[front] !== undefined && !seenCols.has(db)) {
          seenCols.add(db);
          setClauses.push(`${db} = ?`);
          values.push(body[front]);
        }
      }
      values.push(1);
